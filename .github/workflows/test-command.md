...
| `t3000-profiler-tests`, `single-card-tests`, `pipeline-select-profiler` | T3K / single card / selectable | `tt_metal/tools/profiler/**`, tracy, or profiling instrumentation |
...
| `single-card-tests` | Single card | Device-perf regressions from op or kernel changes, profiler instrumentation, cpp unit tests |
...
- **Suite and board toggles: `run-<something>` booleans that default to `true`.** Three
  ...
  | `single-card-tests` | `run-device-perf`, `run-n150-profiler`, `run-n300-profiler`, `run-blackhole-profiler` |
